...
def scrapeRSS( sourceUrl,cat,rate,cursor, categoryList, keywordsList,delimiter):
	...
	print("Scraping ------ ", sourceUrl, " for ", cat)
	for newsItem in feed['items']:
		...
		if len(keyList) > 0:
			...
			if rowCounter==0:
				#print(sqlStr)
				sqlStr = "insert into public.rawAlerts (name,title, alertSource, classification,rawData,keyWords,initialRating, timestamp) values ('" + sourceUrl +"','"+ title + "','"+ alertSource + "','"+ cat + "','" + rawData + "','" + keyString + "','" + str(maxVal) + "','" + timestamp + "')"
				writeToDatabase(cursor, sqlStr)
