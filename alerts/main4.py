...
def scrapeRSS( sourceUrl,cat,rate,cursor, categoryList, keywordsList,delimiter):
	...
	for newsItem in feed['items']:
		...
		if len(keyList) > 0:
			...
			print(newsItem['title'])
			print(keyString)
			sqlStr = "insert into public.rawAlerts (name,alertSource, classification,rawData,keyWords) values ('" + sourceUrl +"','"+ alertSource + "','"+ cat + "','" + rawData + "','" + keyString + "')"
			writeToDatabase(cursor, sqlStr)
